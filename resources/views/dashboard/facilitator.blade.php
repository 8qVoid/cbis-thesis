...
<div class="row g-4 mb-4">
    <div class="col-xl-7">
        ...
        @if($nextEvent)
            ...
            @php($step = $nextEvent->approval_status === 'approved' ? ($nextEvent->is_public ? 4 : 3) : 2)
    ...
    <div class="col-xl-5">
        <section class="card h-100"><div class="card-header cbis-card-title"><span>Proposed Venue Location</span></div><div class="card-body p-0">
            <div class="cbis-map-preview"><div><span>⌖</span><strong>{{ $nextEvent?->venue ?? 'No location proposed' }}</strong><small>{{ $nextEvent?->latitude && $nextEvent?->longitude ? $nextEvent->latitude.', '.$nextEvent->longitude : 'Choose a location when creating an activity.' }}</small></div></div>
