...
<div class="col-md-4"><label class="form-label">First Name</label><input name="first_name" class="form-control js-person-name" value="{{ old('first_name',$donor->first_name) }}" maxlength="80" pattern="[\p{L}\s.'-]+" required></div>
<div class="col-md-4"><label class="form-label">Last Name</label><input name="last_name" class="form-control js-person-name" value="{{ old('last_name',$donor->last_name) }}" maxlength="80" pattern="[\p{L}\s.'-]+" required></div>
<div class="col-md-4"><label class="form-label">Birth Date</label><input type="date" name="birth_date" class="form-control" value="{{ old('birth_date',$donor->birth_date?->toDateString()) }}" required><small class="text-muted">Donor account can be kept before 18; eligibility is blocked until age {{ \App\Support\DonationAgePolicy::MINIMUM_AGE }}.</small></div>
