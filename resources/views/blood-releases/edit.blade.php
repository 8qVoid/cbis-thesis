...
<div class="row g-3">
<div class="col-md-6"><label class="form-label">Inventory</label><select name="blood_inventory_id" class="form-select">@foreach($inventory as $item)<option value="{{ $item->id }}" @selected($bloodRelease->blood_inventory_id==$item->id)>{{ $item->blood_type }} - {{ $item->units_available }} units</option>@endforeach</select></div>
