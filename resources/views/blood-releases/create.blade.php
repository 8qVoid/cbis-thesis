...
<div class="row g-3">
<div class="col-md-6"><label class="form-label">Inventory</label><select name="blood_inventory_id" class="form-select">@foreach($inventory as $item)<option value="{{ $item->id }}">{{ $item->blood_type }} - {{ $item->units_available }} units @if(auth('web')->user()?->isCentralAdmin()) - {{ $item->facility->name ?? 'No facility' }} @endif</option>@endforeach</select></div>
