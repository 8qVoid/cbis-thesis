...
<div class="col-md-4"><label class="form-label">Blood Component</label><select name="component" class="form-select">@foreach(\App\Models\BloodInventory::COMPONENTS as $value=>$label)<option value="{{ $value }}">{{ $label }}</option>@endforeach</select></div>
