...
<div class="mb-3">
    ...
    <p class="cbis-page-subtitle">Update stock details only when a facility correction is needed.</p>
</div>
...
<div class="col-md-4"><label class="form-label">Blood Component</label><select name="component" class="form-select">@foreach(\App\Models\BloodInventory::COMPONENTS as $value=>$label)<option value="{{ $value }}" @selected($bloodInventory->component===$value)>{{ $label }}</option>@endforeach</select></div>
...
<div class="col-12"><button class="btn btn-danger">Update Adjustment</button></div>
</div></form>
@endsection
