...
class StoreDonationScheduleRequest extends BaseFormRequest
{
    ...
    {
        return [
            'facility_id' => [$this->user()?->isQao() ? 'required' : 'nullable', 'integer', \Illuminate\Validation\Rule::exists('facilities', 'id')->where('is_active', true)->whereNull('deleted_at')],
            ...
            'latitude' => [$this->user()?->isQao() ? 'required' : 'nullable', 'numeric', 'between:-90,90'],
            'longitude' => [$this->user()?->isQao() ? 'required' : 'nullable', 'numeric', 'between:-180,180'],
